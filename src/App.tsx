...
import { ActivityOutcome, ExecutionCheckResult, FlowLiftMetrics, FlowType, Screen, PicOSConstraints, StoreInfo } from "./types";
...
export default function App() {
  ...
  const handleConfirmHuntOpportunity = (metrics: FlowLiftMetrics, opportunityId: string) => {
    setFlowType("HUNT_SPACE");
  ...
  const handleSubmitExecutionCheck = (result: ExecutionCheckResult) => {
    ...
    if (result.executed === true) {
      ...
      triggerToast("Execution confirmed and timestamped.", "success");
      return;
    }
    ...
    <div className="w-full h-screen font-sans bg-slate-50 relative select-none">
      ...
      {currentScreen === Screen.HUNT_WORKFLOW && (
        <HuntWorkflow
          ...
          onConfirmOpportunity={handleConfirmHuntOpportunity}
        />
      )}
      ...
      {currentScreen === Screen.AFTER_PHOTO && (
        <PhotoCapture
          ...
          onCancel={() => setCurrentScreen(Screen.OPTIMIZE_DISPLAY)}
        />
      )}
